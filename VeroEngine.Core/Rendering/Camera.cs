...
namespace VeroEngine.Core.Rendering;
    ...
    public class Camera
    {
        ...
        public Camera(Vector3 position, float yaw, float pitch, float roll, Vector3 up, float fieldOfView, float aspectRatio, float nearClip, float farClip)
        {
            _position = position;
            _yaw = yaw;
            _pitch = pitch;
        ...
        public Matrix4 GetViewMatrix()
        {
            ...
            Vector3 right = Vector3.Cross(front, _up).Normalize();
            Vector3 up = Vector3.Cross(right, front).Normalize();
            
            Matrix4 rollMatrix = Matrix4.CreateFromAxisAngle(front.ToOpenTK(), _roll);
